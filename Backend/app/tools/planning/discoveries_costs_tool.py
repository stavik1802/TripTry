"""
Discoveries Costs Tool for TripPlanner Multi-Agent System

This tool combines discovery data with cost inference to provide comprehensive
pricing information for POIs, restaurants, and activities. It estimates costs
based on location, category, and other factors.

Key features:
- Cost inference for discovered POIs and restaurants
- Location-based pricing adjustments
- Category-specific cost estimation
- Date-aware pricing calculations
- Data deduplication and normalization

The tool bridges the gap between discovery and budget planning by providing
realistic cost estimates for all discovered items, enabling accurate budget
calculations and trip optimization.
"""
...
from app.tools.tools_utils.state import AppState  # or stub in tests if unavailable
...
def discovery_and_cost(state: AppState) -> AppState:
    ...
    if not isinstance(req, dict):
        print(f"[ERROR] discoveries_costs_tool - state.request is not a dict, it's {type(req)}: {req}")
