"""
Optimizer Helper Tool for TripPlanner Multi-Agent System

This tool provides itinerary optimization algorithms that create efficient daily
schedules by considering distances, travel times, opening hours, and user preferences.
It uses greedy algorithms to maximize the number of activities while minimizing travel time.

Key features:
- Greedy optimization algorithm for daily itineraries
- Distance and time-based activity sequencing
- Meal scheduling integration
- Transportation mode optimization (walking vs. taxi)
- Time window management for POIs and restaurants

The tool creates optimized daily schedules that maximize the value of each day
while considering practical constraints like travel time and opening hours.
"""
...
def itinerary_optimizer_greedy(state: AppState) -> AppState:
    ...
    if not isinstance(req, dict):
        print(f"[ERROR] optimizer_tool - state.request is not a dict, it's {type(req)}: {req}")
