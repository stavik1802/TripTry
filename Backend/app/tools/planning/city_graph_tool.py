"""
City Graph Tool for TripPlanner Multi-Agent System

This tool assembles geographical and cost data for cities, creating comprehensive
graph structures that include POIs, restaurants, transportation options, and
estimated costs. It calculates distances, travel times, and cost estimates.

Key features:
- Geographic data assembly and graph construction
- Distance and travel time calculations
- Cost estimation for transportation and activities
- Fallback distance calculations when coordinates are missing
- Speed-based travel time estimates (walking, driving, public transport)

The tool creates the foundation for trip optimization by providing detailed
geographical and cost information that enables efficient itinerary planning.
"""
...
def geocost_assembler(state: AppState) -> AppState:
    ...
    # Defensive check: ensure req is a dictionary
    if not isinstance(req, dict):
        print(f"[ERROR] city_graph_tool - state.request is not a dict, it's {type(req)}: {req}")
        state.meta["error"] = f"Expected dict but got {type(req)}"
        state.logs = logs; return state
    
    print(f"[DEBUG] City graph tool - state type: {type(state)}")
    print(f"[DEBUG] City graph tool - state.request type: {type(req)}")
    print(f"[DEBUG] City graph tool - state.request keys: {list(req.keys()) if isinstance(req, dict) else 'Not a dict'}")
    print(f"[DEBUG] City graph tool - discovery type: {type(req.get('discovery'))}")
    print(f"[DEBUG] City graph tool - discovery value: {req.get('discovery')}")
    
    disc = req.get("discovery") or {}
    print(f"[DEBUG] City graph tool - disc type: {type(disc)}")
    print(f"[DEBUG] City graph tool - disc keys: {list(disc.keys()) if isinstance(disc, dict) else 'Not a dict'}")
    
    # Additional defensive check for disc
    if not isinstance(disc, dict):
        print(f"[ERROR] city_graph_tool - discovery is not a dict, it's {type(disc)}: {disc}")
        state.meta["error"] = f"Expected discovery to be dict but got {type(disc)}"
        state.logs = logs; return state
